...
import TextField from "@mui/material/TextField";
import { useSelector, useDispatch } from "react-redux";
import { GetAllPatients, DeletePatient, StatusPatient } from "../reducer/PatientsSlice";
import { useNavigate } from "react-router-dom";
import Swal from "sweetalert2";
import { GetAllTreatment } from "../reducer/TreatmentSlice";
import axios from "axios";
...
export default function Reports() {
  ...
  const handleClickOpen = async (e, id) => {
    e.preventDefault();
    try {
      await dispatch(StatusPatient({ id, status: Number(seekerStatus) })).unwrap();
      Swal.fire("Success!", "Patient details updated successfully.", "success");
      dispatch(GetAllPatients());
    } catch (err) {
      Swal.fire("Error!", err?.message || "An error occurred", "error");
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className="search-btn-main mb-4">
              <div className="mr-2 field-count">
                <DatePicker
                  value={dateRange}
                  format="MM/DD/YYYY"
                  placeholder="Start Date To End Date"
              ...
              <div className="table-responsive">
                <TableContainer component={Paper} style={{ overflowX: "auto" }}>
                  <Table stickyHeader aria-label="sticky table" className="table-no-card">
                    ...
                    <TableBody>
                      {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((info, i) => {
                        console.log(info)
                        return (
                          <>
                            <TableRow role="checkbox" tabIndex={-1} key={info.patientId}>
                              <TableCell>{page * rowsPerPage + i + 1}</TableCell>
                              <TableCell>{info.patientId}</TableCell>
                              <TableCell>{info.patient_name}</TableCell>
                              <TableCell>{info.emergency_contact ?  info.emergency_contact    :info.emergency_contact_no}</TableCell>
                              <TableCell>{info.email}</TableCell>
                              <TableCell>{info.country}</TableCell>
                              <TableCell>{info.treatment_course_name}</TableCell>
                            </TableRow>
                          </>
                        )
                      })}
