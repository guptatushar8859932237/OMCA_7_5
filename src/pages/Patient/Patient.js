  ...
  const handleFilter = (event) => {
    if (event.target.value === "") {
      ...
      const filterResult = searchApiData.filter((item) => {
        ...
        const contact = item?.emergency_contact ? item.emergency_contact.toString().toLowerCase() : "";
        const country = item.country?.toLowerCase() || "";
        const patientdesiese = item.patient_disease[0].disease_name?.toLowerCase() || "";
        const searchValue = event.target.value.toLowerCase();
        return (
          enquiryId.includes(searchValue) ||
          country.includes(searchValue) || patientdesiese.includes(searchValue) || contact.includes(searchValue) || name.includes(searchValue) || emailMatches.includes(searchValue)
        );
      });
